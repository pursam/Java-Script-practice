...
function getLocalDate(date, isSeconds, isISO) {
  ...
  let dt = date.getDate() <= 9 ? `0${date.getDate()}` : date.getDate();
  ...
  let isostr = `${date.getFullYear()}-${month}-${dt}, ${hours}:${minutes}:${seconds}`;
  return isSeconds === true ?
    ...
    : isISO === true ?
      isostr.slice(0, -3)
      : date.toLocaleString().toString().slice(0, -3);
}
...
function parseDate(dateStr) { // функция для избежания повторов
  let dateObj = new Date(0)
  dateObj.setYear(dateStr.slice(0, 4))
  dateObj.setMonth(dateStr.slice(5, 7) - 1)
  dateObj.setDate(dateStr.slice(8, 10))
  return dateObj;
}

function getWeekDay(date) {
  let day = parseDate(date).getDay()
...
function getLocalDay(date) {
  ...
  let day = dateObj.getDay() === 0 ? 7 : dateObj.getDay();
...
function getDateAgo(date, days) {
  let msNow = Date.parse(parseDate(date));
  let msdays = days * 8.64e+7
  let dateObj = new Date(msNow - msdays)
...
Car = function (engine, model, name, year) {
  ...
  this.name = name;
  this.year = year;
  this.info = () => `${this.name} ${this.model}, ${this.engine}cc, year ${this.year}, ${this.used}`;
  // this.used = this.year !== new Date(Date.now()).getFullYear() ? 'used' : 'new';
}
