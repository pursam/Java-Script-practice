...
var counting = {
  count: 0,
  value(n) {
    return n !== undefined ? this.count = n : this.count;
  },
  increment() {
    return this.count++
  },
...
function myPow(a, b, callback) {
  var res = pow(a, b)
  function pow(a, b) {
    res = b === 1 ? a : a * pow(a, b - 1);
    return res;
  }
  return callback(a, b, res);
}

let myPrint = (f, s, c) => `${f}^${s}=${c}`; // f^s=c;
...
function fullInfo() {
  arr = `${this.name} ${this.model}, ${this.engine}cc, year ${this.year}, ${this.used}`;
  return arr;
};
